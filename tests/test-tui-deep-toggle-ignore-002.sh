...
# test-tui-deep-toggle-ignore-002.sh - Deep toggle with gitignore active and guardignore inactive
...
test_deep_toggle_with_gitignore_active_guardignore_inactive() {
    log_test "test_deep_toggle_with_gitignore_active_guardignore_inactive" \
             "Enter recursively toggles txt and tmp files when only gitignore is active"

    deep_create_fixture true
    deep_init_guard
    "$GUARD_BIN" config set use_gitignore true
    ...
    deep_assert_command_files_guard_flag "true" deep_guardignored_files
...
tui_run_test test_deep_toggle_with_gitignore_active_guardignore_inactive
