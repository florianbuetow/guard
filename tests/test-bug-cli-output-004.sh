...
# test-bug-cli-output-004.sh - BUG: enable collection output order and header
#
...
# ==========================================================================
# BUG: enable collection output order
# ==========================================================================

test_enable_collection_output_header_and_order() {
    log_test "test_enable_collection_output_header_and_order" \
             "Enable collection should print header, sorted files, blank line, then summary"
    ...
    for i in "${!lines[@]}"; do
        ...
        if [[ "$line" == "Guard enabled for apple.txt" ]]; then
            apple_line=$line_no
        fi
        if [[ "$line" == "Guard enabled for banana.txt" ]]; then
            banana_line=$line_no
        fi
        if [[ "$line" == "Guard enabled for cherry.txt" ]]; then
            cherry_line=$line_no
        fi
        if [[ "$line" == "Guard enabled for collection fruits" ]]; then
...
# Run test
run_test test_enable_collection_output_header_and_order
